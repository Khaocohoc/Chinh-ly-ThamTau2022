...
library(tidyverse)
library(readxl)
cch <- read_xlsx("Congcuhach.xlsx", sheet = "Congcuhach")
cch
cch %>% 
  group_by(`Loaihinh`) %>%
  tally()
cch_Loaihinh_tally <-  cch %>% 
  group_by(`Loaihinh`) %>% 
  tally() %>% 
  filter(`Loaihinh` != 'NA') %>%  # REMOVE NAs 
  arrange(desc=(n))
ggplot(cch_Loaihinh_tally,
...
cch
library(ggplot2)
ggplot(data=cch, aes(x=Loaihinh, y="")) + 
...
# Phan bo theo lop và O khai quat
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
