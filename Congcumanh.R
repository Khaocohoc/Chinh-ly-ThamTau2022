# MANH TUOC DA VOI - Hinh THAI MAT CAT NGANG
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
ccm <- read_xlsx("Congcuhach.xlsx", sheet = "Ccmanh")
ccm
ccm %>% 
  group_by(`Matcatngang`) %>%
  tally()
ccm_Matcatngang_tally <-  ccm %>% 
  group_by(`Matcatngang`) %>% 
  tally() %>% 
  filter(`Matcatngang` != 'NA') %>%  # REMOVE NAs 
  arrange(desc=(n))
ggplot(ccm_Matcatngang_tally,
       aes(x = reorder(`Matcatngang`, n), y = n)) +
  geom_bar(stat="identity", size = 0.7, 
           fill = "white", color = "black", 
           width = 0.8) +
  theme_bw() +
  theme(text = element_text(size=25, colour = 'black'),   # cach tang kich thuoc SIZE CHU
        axis.text.y = element_text(size = , colour = 'black')) +
  xlab("") +
  ylab("Tần suất") +
  coord_flip()
...
# MANH TUOC DA VOI - Hinh THAI MAT CAT DOC
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
...
ccm %>% 
  group_by(`Matcatdoc`) %>%
  tally()
ccm_Matcatdoc_tally <-  ccm %>% 
  group_by(`Matcatdoc`) %>% 
  tally() %>% 
  filter(`Matcatdoc` != 'NA') %>%  # REMOVE NAs 
  arrange(desc=(n))
ggplot(ccm_Matcatdoc_tally,
       aes(x = reorder(`Matcatdoc`, n), y = n)) +
  geom_bar(stat="identity", size = 0.7, 
           fill = "white", color = "black", 
           width = 0.8) +
  theme_bw() +
...
ccm
library(ggplot2)
ggplot(data=cch, aes(x=Tinhtrang, y="")) + 
  geom_bar(stat="identity", colour = 'black') +
  facet_grid(~ Toado) +
  theme_bw() +
  labs(title="", x="", y="", fill="", colour = 'black') + 
  theme(text = element_text(size = 25, angle = 0, colour = 'black')) +
  theme(plot.title = element_text(size=20, colour = 'black', margin=margin(t= 20, b=2))) +
  coord_flip()


###--------------------------------------------------------------------------------------
# QUY MO CAC CONG CU MANH_THAM TAU 2022
...
ccm <- read_xlsx("Congcuhach.xlsx", sheet = "Ccmanh")
ccm
ggplot(ccm, aes(x = reorder(Tinhtrang, Trongluong),
                 y = Trongluong)) +
...
ccm <- read_xlsx("Congcuhach.xlsx", sheet = "Ccmanh")
ccm
ggplot(ccm, aes(x = reorder(Tinhtrang, Dai),
                 y = Dai)) +
  geom_boxplot(size = 0.8, width = 0.6) +
  geom_point(size = 3, alpha = 1/2) +
  theme_bw() +
...
ccm <- read_xlsx("Congcuhach.xlsx", sheet = "Ccmanh")
ccm
ggplot(ccm, aes(x = reorder(Tinhtrang, Rong),
                 y = Rong)) +
  geom_boxplot(size = 0.8, width = 0.6) +
  geom_point(size = 3, alpha = 1/2) +
  theme_bw() +
...
ccm <- read_xlsx("Congcuhach.xlsx", sheet = "Ccmanh")
ccm
ggplot(ccm, aes(x = reorder(Tinhtrang, Day),
                y = Day)) +
  geom_boxplot(size = 0.8, width = 0.6,
               palette = "jco") +
  geom_point(size = 3, alpha = 1/2) +
...
# Phan bo nguyen lieu theo Loai hinh-Goc ghe trong
  ...
  ccm <- read_xlsx("Congcuhach.xlsx", sheet = "Ccmanh")
  ccm
ggplot(ccm, aes(x = reorder(Tinhtrang, Gocluoi1),
                 y = Gocluoi1)) +
  geom_boxplot(size = 0.8, width = 0.6) +
  geom_point(size = 3, alpha = 1/2) +
  theme_bw() +
...
attach(ccm)
  ...
  ccm
  library(ggplot2)
  ggplot(data=ccm, aes(x=Huonggheriatacdung, y="")) + 
    geom_bar(stat="identity", colour = 'black') +
    facet_grid(~ Lop) +
    theme_bw() +
    labs(title="", x="", y="", fill="", colour = 'black') + 
...
# Phan bo nguyen lieu theo Loai hinh-So vet ghe- Cuong do ghe deo
  library(ggplot2)
  library(dplyr)
  library(tidyverse)
  library(readxl)
  ccm <- read_xlsx("Congcuhach.xlsx", sheet = "Ccmanh")
  ccm
  ggplot(ccm, aes(x = reorder(Tinhtrang, Sovetghe),
                 y = Sovetghe)) +
  geom_boxplot(size = 0.8, width = 0.6) +
  geom_point(size = 3, alpha = 1/2) +
  theme_bw() +
  ...
  # Phan bo nguyen lieu theo Loai hinh-So lop ghe
  library(ggplot2)
  library(dplyr)
  library(tidyverse)
  library(readxl)
  ccm <- read_xlsx("Congcuhach.xlsx", sheet = "Ccmanh")
  ccm
  ggplot(ccm, aes(x = reorder(Tinhtrang, Lopghe),
                  y = Lopghe)) +
    geom_boxplot(size = 0.8, width = 0.6) +
    geom_point(size = 3, alpha = 1/2) +
    theme_bw() +
