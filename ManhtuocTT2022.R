...
library(tidyverse)
library(readxl)
mt <- read_xlsx("Manhtuoc.xlsx", sheet = "FlakesF12_13")
mt
mt %>% 
  group_by(`Toado`) %>%
  tally()
mt_Toado_tally <- mt %>% 
  group_by(`Toado`) %>% 
  tally() %>% 
  filter(`Toado` != 'NA') %>%  # REMOVE NAs 
  arrange(desc=(n))
ggplot(mt_Toado_tally,
       aes(x = reorder(`Toado`, n), y = n)) +
...
mt
library(ggplot2)
ggplot(data=mt, aes(x=Tinhtrang, y="")) + 
  geom_bar(stat="identity", colour = 'black') +
  facet_grid(~Toado) +
  theme_bw() +
  labs(title="", x="", y="", fill="", colour = 'black') + 
  theme(text = element_text(size = 15, angle = 0, colour = 'black')) +
  theme(plot.title = element_text(size= 25, colour = 'black', margin=margin(t=30, b=2))) +
  coord_flip()



# Phan bo theo lop _ MAU SAC
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
...
# VI TRI VO CUOI TREN LUNG_THAM TAU 2022
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
...
mt
library(ggplot2)
ggplot(data=mt, aes(x=Vitrivicuoi, y="")) + 
  geom_bar(stat="identity", colour = 'black') +
  facet_grid(~Toado) +
  theme_bw() +
  labs(title="", x="", y="", fill="", colour = 'black') + 
...
#-------------------------------------------------------------------------------
# KIEM DINH CAC GIA TRI VE QUY MO - TRONG LUONG, DAI, RONG, DAY
...
mt <- read_xlsx("Manhtuoc.xlsx", sheet = "FlakesF12_13")
mt
ggplot(mt, aes(x = reorder(Tinhtrang, Trongluong),
                 y = Trongluong)) +
...
max(mt$Trongluong)
attach(mt)
ggplot(mt, aes(x = Tinhtrang, y = Trongluong)) + 
  geom_boxplot() +
  coord_flip()
...
mt <- read_xlsx("Manhtuoc.xlsx", sheet = "FlakesF12_13")
mt
ggplot(mt, aes(x = reorder(Tinhtrang, Dai),
                 y = Dai)) +
  geom_boxplot(size = 0.8, width = 0.6) +
  geom_point(size = 3, alpha = 1/2) +
  theme_bw() +
...
max(mt$Dai)
attach(mt)
ggplot(mt, aes(x = Tinhtrang, y = Dai)) + 
  geom_boxplot() +
  coord_flip()
av = aov(Dai ~ Tinhtrang)
summary(av)
av
TukeyHSD(av)
Dai = TukeyHSD(av)
...
mt <- read_xlsx("Manhtuoc.xlsx", sheet = "FlakesF12_13")
mt
ggplot(mt, aes(x = reorder(Tinhtrang, Rong),
                 y = Rong)) +
  geom_boxplot(size = 0.8, width = 0.6) +
  geom_point(size = 3, alpha = 1/2) +
  theme_bw() +
...
max(mt$Rong)
attach(mt)
ggplot(mt, aes(x = Tinhtrang, y = Rong)) + 
  geom_boxplot() +
  coord_flip()
av = aov(Rong ~ Tinhtrang)
summary(av)
av
TukeyHSD(av)
Rong = TukeyHSD(av)
...
mt <- read_xlsx("Manhtuoc.xlsx", sheet = "FlakesF12_13")
mt
ggplot(mt, aes(x = reorder(Tinhtrang, Day),
                y = Day)) +
  geom_boxplot(size = 0.8, width = 0.6,
               palette = "jco") +
  geom_point(size = 3, alpha = 1/2) +
...
max(mt$Day)
attach(mt)
ggplot(mt, aes(x = Tinhtrang, y = Day)) + 
  geom_boxplot() +
  coord_flip()
av = aov(Day ~ Tinhtrang)
summary(av)
av
TukeyHSD(av)
Day = TukeyHSD(av)
...
mt <- read_xlsx("Manhtuoc.xlsx", sheet = "FlakesF12_13")
mt
ggplot(mt, aes(x = reorder(Tinhtrang, Rongdienghe),
                 y = Rongdienghe)) +
  geom_boxplot(size = 0.8, width = 0.6) +
  geom_point(size = 3, alpha = 1/2) +
  theme_bw() +
...
max(mt$Rongdienghe)
attach(mt)
ggplot(mt, aes(x = Tinhtrang, y = Rongdienghe)) + 
  geom_boxplot() +
  coord_flip()
av = aov(Rongdienghe ~ Tinhtrang)
summary(av)
av
TukeyHSD(av)
Rongdienghe = TukeyHSD(av)
plot(Rongdienghe)



# Phan bo nguyen lieu theo Loai hinh- DAY DIEN GHE
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
mt <- read_xlsx("Manhtuoc.xlsx", sheet = "FlakesF12_13")
mt
ggplot(mt, aes(x = reorder(Tinhtrang, Daydienghe),
               y = Daydienghe)) +
  geom_boxplot(size = 0.8, width = 0.6) +
  geom_point(size = 3, alpha = 1/2) +
  theme_bw() +
...
max(mt$Daydienghe)
attach(mt)
ggplot(mt, aes(x = Tinhtrang, y = Daydienghe)) + 
  geom_boxplot() +
  coord_flip()
av = aov(Daydienghe ~ Tinhtrang)
summary(av)
av
TukeyHSD(av)
Daydienghe = TukeyHSD(av)
plot(Daydienghe)
...
mt %>% 
  group_by(`Tylevocuoi`) %>%
  tally()
mt_Tylevocuoi_tally <- mt %>% 
  group_by(`Tylevocuoi`) %>% 
  tally() %>% 
  filter(`Tylevocuoi` != 'NA') %>%  # REMOVE NAs 
  arrange(desc=(n))
ggplot(mt_Tylevocuoi_tally,
       aes(x = reorder(`Tylevocuoi`, n), y = n)) +
  geom_bar(stat="identity", size = 0.7, 
           fill = "white", color = "black", 
           width = 0.8) +
  theme_bw() +
  theme(text = element_text(size=25, colour = 'black'),   # cach tang kich thuoc SIZE CHU
        axis.text.y = element_text(size = 25, colour = 'black')) +
  xlab("Tỷ lệ vỏ cuội mặt lưng") +
  ylab("Tần suất")


# HINH THAI DAU VA DUOI MANH TUOC_THAM TAU 2022
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
mt <- read_xlsx("Manhtuoc.xlsx", sheet = "FlakesF12_13")
mt_no_na <- mt %>%
  drop_no_na(Hinhthaidau)
  mt_no_na <- mt %>% 
  drop_na(Hinhthaidau) %>%
  mutate(Toado = factor(Toado, levels = c("L1.f12", "L1.f13", 
                                            "L2.f12", 
                                            "L3.f12", "L3.f13", 
                                            "L4.f12", "L4.f13", 
                                            "L5.f12", "L5.f13", 
                                            ...
                                            "L8.f12", "L8.f13",
                                            "L9.f13", "L11.f13")))
ggplot(data=mt_no_na, aes(x=Hinhthaidau, y="")) + 
  geom_bar(stat="identity", colour = 'black') +
  facet_grid(~ Toado) +
  theme_bw() +
  labs(title="", x="", y="", fill="", colour = 'black') + 
  theme(text = element_text(size = 20, angle = 0, colour = 'black')) +
  theme(plot.title = element_text(size= 25, colour = 'black', margin=margin(t=30, b=2))) +
  coord_flip()
...
mt
library(ggplot2)
ggplot(data=mt, aes(x=Loaidienghe, y="")) + 
  geom_bar(stat="identity", colour = 'black') +
  facet_grid(~Toado) +
  theme_bw() +
  labs(title="", x="", y="", fill="", colour = 'black') + 
...
#-------------------------------------------------------------------------------
# RAT HAY_Luu y va Removing NAs for ggplot2 (LUU Y)
# HINH THAI DAU_MANH TUOC
library(ggplot2)
library(dplyr)
library(tidyverse)
library(readxl)
mt <- read_xlsx("Manhtuoc.xlsx", sheet = "FlakesF12_13")
mt %>% 
  group_by(`Hinhthaidau`) %>%
  tally()
mt_Hinhthaidau_tally <- mt %>% 
  group_by(`Hinhthaidau`) %>% 
  tally() %>% 
  filter(`Hinhthaidau` != 'NA') %>%  # REMOVE NAs 
  arrange(desc=(n))
ggplot(mt_Hinhthaidau_tally,
       aes(x = reorder(`Hinhthaidau`, n), y = n)) +
  geom_bar(stat="identity", size = 0.7, 
           fill = "white", color = "black", 
           width = 0.8) +
  theme_bw() +
...
mt
mt %>% 
  group_by(`Hinhthaiduoi`) %>%
  tally()
mt_Hinhthaiduoi_tally <- mt %>% 
  group_by(`Hinhthaiduoi`) %>% 
  tally() %>% 
  filter(`Hinhthaiduoi` != 'NA') %>%  # REMOVE NAs 
  arrange(desc=(n))
ggplot(mt_Hinhthaiduoi_tally,
       aes(x = reorder(`Hinhthaiduoi`, n), y = n)) +
  geom_bar(stat="identity", size = 0.7, 
           fill = "white", color = "black", 
           width = 0.8) +
  theme_bw() +
...
mt
str(mt)
ggplot(mt, aes(x = reorder(Hinhthaidau, Gocghengoai),
                y = Gocghengoai)) +
  geom_boxplot(size = 0.8, width = 0.6,
               palette = "jco") +
  geom_point(size = 3, alpha = 1/2) +
...
mt
attach(mt)
ggplot(mt, aes(x = Hinhthaidau, y = Gocghengoai)) + 
  geom_boxplot() +
  coord_flip()
av = aov(Gocghengoai ~ Hinhthaidau)
summary(av)
av
TukeyHSD(av)
Gocghe1 = TukeyHSD(av)
...
mt
str(mt)
ggplot(mt, aes(x = reorder(Hinhthaidau, Gocghetrong),
               y = Gocghetrong)) +
  geom_boxplot(size = 0.8, width = 0.6,
               palette = "jco") +
  geom_point(size = 3, alpha = 1/2) +
...
mt
attach(mt)
ggplot(mt, aes(x = Hinhthaidau, y = Gocghetrong)) + 
  geom_boxplot() +
  coord_flip()
av = aov(Gocghetrong ~ Hinhthaidau)
summary(av)
av
TukeyHSD(av)
Gocghe2 = TukeyHSD(av)
...
# Tong so vet ghe _ Hinh thai dau
mt
attach(mt)
ggplot(mt, aes(x = Hinhthaidau, y = Tongsovetghe)) + 
  geom_boxplot() +
  coord_flip()
av = aov(Tongsovetghe ~ Hinhthaidau)
summary(av)
av
TukeyHSD(av)
Sovet = TukeyHSD(av)
...
# Tong so vet ghe _ Hinh thai dduoi
mt
attach(mt)
ggplot(mt, aes(x = Tinhtrang, y = Tylevocuoi)) + 
  geom_boxplot() +
  coord_flip()
av = aov(Tylevocuoi ~ Tinhtrang)
summary(av)
av
TukeyHSD(av)
Tyle = TukeyHSD(av)
...
str(hd1)
names(hd1)
ggplot(hd1, aes(x = reorder(Loaihinh, D),
                y = Dai)) +
  geom_boxplot(size = 0.8, width = 0.6,
               palette = "jco") +
  geom_point(size = 3, alpha = 1/2) +
